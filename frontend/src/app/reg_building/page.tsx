...
export default function RegBuildingPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    let url = `http://localhost:5000/api/properties/registered?Type=2`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.Msg || 'Failed to retrieve building property portfolio.');
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="space-y-6">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white flex items-center gap-3">
            ...
            Registered Building Property
          </h2>
          <p className="text-slate-400 text-sm mt-1">View and track all building properties registered under townships</p>
        </div>
        ...
        <form onSubmit={handleSearch} className="p-6 bg-slate-900 border border-slate-800 rounded-xl shadow-lg space-y-4">
          ...
          <div className="flex justify-end pt-2">
            <button
              type="submit"
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-lg text-sm transition-all flex items-center gap-2 cursor-pointer shadow-md"
            >
              <Search size={16} />
            ...
            <div className="overflow-x-auto">
              {loading ? (
                <div className="py-20 flex flex-col items-center justify-center gap-3 text-slate-400">
                  ...
                  <span>Loading registered building records...</span>
                </div>
              ) : properties.length === 0 ? (
                <div className="py-20 text-center text-slate-550 text-sm font-medium">
                  No registered building properties found matching these filter credentials.
                </div>
              ) : (
