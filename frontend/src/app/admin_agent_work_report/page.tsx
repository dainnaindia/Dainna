...
export default function AdminAgentWorkReportPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      let url = `http://localhost:5000/api/reports/agent-work-report?startdate=${startdate}&enddate=${enddate}&property_type=${propertyType}&area=${area}&category=${category}&sorting_by=${sortingBy}`;
  ...
  const exportToCSV = () => {
    const headers = ['#', 'Agent Name', 'Project Name', 'Size (Sq.Ft)', 'Total Amount'];
    ...
    const csvContent = [
        `"Admin Agent Work Report from ${startdate} to ${enddate}"`,
        '',
        headers.join(','), 
    ...
    link.setAttribute("download", `admin_agent_work_report_${new Date().toISOString().slice(0,10)}.csv`);
    ...
    <DashboardLayout role="admin">
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center print:hidden">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight text-white">Agent Work Report</h2>
            <p className="text-slate-400 text-sm mt-1">
              Admin interface to analyze total work size and earnings aggregate grouped by agent and project.
            </p>
          </div>
        ...
        <div className="hidden print:block text-center border-b border-black pb-4 mb-6">
          <h2 className="text-xl font-bold text-black">Agent Work Report (Admin)</h2>
            ...
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 print:grid-cols-2">
                  ...
                  <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl print:border-black print:bg-white">
                    <span className="block text-slate-500 text-[10px] uppercase font-bold tracking-wider print:text-black font-semibold">Total Amount (₹)</span>
                      ...
                      <tbody className="divide-y divide-slate-800/60 print:divide-black">
                        {records.map((r, idx) => (
                          <tr key={idx} className="hover:bg-slate-950/40 transition-colors print:hover:bg-transparent">
                            ...
                            <td className="py-3.5 px-4 font-semibold text-white print:text-black">{r.agentName}</td>
