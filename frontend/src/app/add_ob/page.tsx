...
import { Landmark, Plus, Trash2, ArrowRight, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';

export default function AddOpenBuildingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/properties/add-ob', {
        method: 'POST',
      ...
      if (data.Status === 2) {
        setSuccess('Open Building record inserted successfully! Redirecting...');
        setTimeout(() => {
          router.push(`/add_agreement_draft?oid=${data.OLBID}`);
        }, 1500);
      } else {
        setError(data.Msg || 'Failed to insert Open Building record.');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <DashboardLayout role="agent">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white">Add New Open Building</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
            ...
            <h3 className="font-bold text-lg text-white">Open Building Specifics</h3>
          </div>
          ...
          <div className="flex justify-end pt-4 border-t border-slate-800">
            ...
            >
              {loading ? (
                ...
                <>
                  <span>Create Open Building</span>
                  <ArrowRight size={16} />
                </>
              )}
            </button>
