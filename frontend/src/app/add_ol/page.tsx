...
import { FileSpreadsheet, Plus, Trash2, ArrowRight, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function AddOpenLandPage() {
  ...
  const [stateId, setStateId] = useState('12'); // Example state ID
  ...
  // Dynamic Survey Rows
  const [surveyRows, setSurveyRows] = useState<SurveyRow[]>([{ newNo: '', oldNo: '' }]);

  const addSurveyRow = () => {
    setSurveyRows([...surveyRows, { newNo: '', oldNo: '' }]);
  };

  const removeSurveyRow = (index: number) => {
    if (surveyRows.length > 1) {
      setSurveyRows(surveyRows.filter((_, i) => i !== index));
    }
  };

  const handleSurveyChange = (index: number, field: 'newNo' | 'oldNo', value: string) => {
    const updated = [...surveyRows];
    updated[index][field] = value;
    setSurveyRows(updated);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/properties/add-ol', {
        method: 'POST',
      ...
      if (data.Status === 2) {
        setSuccess('Open Land record inserted successfully! Redirecting...');
        setTimeout(() => {
          router.push(`/add_agreement_draft?oid=${data.OLBID}`);
        }, 1500);
      } else {
        setError(data.Msg || 'Failed to insert Open Land record.');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <DashboardLayout role="agent">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white">Add New Open Land</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
            ...
            <h3 className="font-bold text-lg text-white">Open Land Specifics</h3>
          </div>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white placeholder-slate-700 text-sm"
                  />
                </div>
                <div>
