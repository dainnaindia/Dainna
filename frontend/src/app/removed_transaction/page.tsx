...
import { Landmark, Search, Loader2, Trash2, Printer, FileSpreadsheet, Copy } from 'lucide-react';
...
export default function RemovedTransactionPage() {
  ...
  // Single Verification Modal state (for viewing details)
  ...
  const fetchInvoices = async (sId = '', c = '', pId = '') => {
    ...
    setError('');
    try {
      let url = 'http://localhost:5000/api/billing/invoices?status=5';
      const params = [];
  ...
  const copyTable = () => {
    const text = filteredInvoices.map((inv, idx) => 
      `${idx + 1}\t${inv.invNo || inv.invoiceNo}\t${new Date(inv.addeddate).toLocaleDateString('en-GB').replace(/\//g, '-')}\t${inv.agentName}\t${inv.projectName}\t₹${inv.grandtotal.toFixed(2)}\t${inv.utr}`
    ).join('\n');
  ...
  return (
    <DashboardLayout role="admin">
      <div className="absolute top-10 right-10 w-[400px] h-[400px] rounded-full bg-slate-500/5 blur-[100px] pointer-events-none print:hidden" />
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center print:hidden">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Review transaction logs and billings that were explicitly deleted by users</p>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="py-20 text-center text-slate-500 text-sm">
                No removed transactions were found.
              </div>
            ) : (
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {filteredInvoices.map((inv, idx) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-right font-mono text-[11px] text-slate-400">₹{inv.sgstAmount.toFixed(2)}</td>
                      <td className="py-3 px-3 text-right font-mono text-[11px] text-slate-400">₹{inv.cgstAmount.toFixed(2)}</td>
      ...
      {/* Payment Reference Modal (for viewing details) */}
