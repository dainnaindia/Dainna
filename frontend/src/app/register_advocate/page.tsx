...
import { Phone, MapPin, Mail, ArrowUp, Send, CheckCircle, AlertCircle, Loader, User, Lock } from 'lucide-react';
import Header from '@/components/Header';

function RegisterAdvocateForm() {
  ...
  // Enforce/fetch project details when pid is in query string
  useEffect(() => {
    if (!pid) {
      router.push('/');
      return;
    ...
    async function loadProject() {
      try {
        const response = await fetch(`http://localhost:5000/api/projects/public/${pid}`);
        const data = await response.json();
        if (response.ok && data.Status === 100 && data.Project) {
          setProjectName(data.Project.projectName);
          setProjectEmail(data.Project.email || '');
          setCity(data.Project.city || '');
          setWorkingCity(data.Project.city || '');
          setStateId(String(data.Project.stateId || ''));
        } else {
          setErrorMsg(data.Msg || 'Failed to retrieve project details.');
        }
      } catch (err) {
        console.error('Error fetching public project:', err);
        setErrorMsg('Failed to connect to API server for project details.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Verify email matches the project's registered email
    if (hasPid && email.trim().toLowerCase() !== projectEmail.trim().toLowerCase()) {
      setErrorMsg('Your Email is Not Registered.');
      // Scroll container to top so the user sees the error
    ...
    const payload = {
      ...
      ProjectID: hasPid ? parseInt(pid as string) : null
    };
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-12 z-10 flex-1 flex flex-col justify-center">
        <div className="bottom40 mb-10 text-center">
          <h1 className="text-3xl font-extrabold text-white tracking-wide uppercase">
            Advocate <span className="text-red-500">Registration</span>
          </h1>
        ...
        <div className="w-full bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 md:p-10 shadow-2xl backdrop-blur-md relative overflow-hidden">
            ...
            <div className="mb-8 p-5 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 text-sm flex items-start gap-3.5 animate-fadeIn">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Your advocate account has been registered with code <strong className="text-green-300 font-mono text-sm">{assignedCode}</strong>. 
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Project Banner */}
            {hasPid && projectName && (
              <div className="p-4 rounded-lg bg-red-500/5 border border-red-500/15 text-slate-400 text-xs font-bold uppercase tracking-wider flex items-center justify-between">
                <span>Project Name : <strong className="text-white normal-case font-extrabold">{projectName}</strong></span>
                <span className="text-[10px] text-red-500 font-bold bg-red-500/10 px-2 py-0.5 rounded">Linked Registration</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              <div>
                ...
                <input
                  ...
                  readOnly={hasPid}
                  className={`w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-lg text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-red-500/15 transition-all text-xs ${
                    hasPid ? 'opacity-65 select-none focus:ring-0 hover:border-slate-800' : 'hover:border-slate-700 focus:border-red-500'
                  }`}
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  disabled={hasPid}
                  className={`w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-slate-300 focus:outline-none focus:ring-2 focus:ring-red-500/15 transition-all text-xs ${
                    hasPid ? 'opacity-65 cursor-not-allowed hover:border-slate-800' : 'hover:border-slate-700 focus:border-red-500'
                  }`}
                >
...
export default function RegisterAdvocatePage() {
  return (
    <Suspense fallback={
