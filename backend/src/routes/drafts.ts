...
router.post('/accept', authMiddleware, async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    // Purchaser notifications (SMS and Email)
    const pFirstName = d.purchaserFirstName || '';
    const pLastName = d.purchaserLastName || '';
    const purchaserName = `${pFirstName} ${pLastName}`.trim() || 'Purchaser';
    const purchaserMobile = d.purchaserMobileNo;
